...
class _DatePickerHeader extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    headerTextTheme.titleMedium?.copyWith(color: Colors.white70, height: 1.4);
...
class HijriDayPicker extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    labels.addAll(
        _getDayHeaders(context, TextStyle(color: Colors.white), localizations, ));
      ...
      if (day < 1) {
        ...
        if (isSelectedDay) {
          ...
          decoration = const BoxDecoration(
              color: AppColors.colorPrimaryLight, shape: BoxShape.circle);
        } else if (disabled) {
